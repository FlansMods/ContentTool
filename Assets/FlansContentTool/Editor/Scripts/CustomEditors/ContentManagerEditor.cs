...
{	
	...
	public void ImportTab(ContentManager instance)
	{
		...
		GUILayout.Space(16);
		FlanStyles.LargeButtonArray(
			new GUIContent[] { 
				FlanStyles.GoToFolder, 
				FlanStyles.GoToModelFolder,
				FlanStyles.OpenDocumentation,
			},
			new string[] { 
				"Open Import Folder", 
				"Open Models Folder",
				"Open Documentation [Github.com]",
			},
			new FlanStyles.ActionFunc[] {
				() => { FlansModToolbox.CreateAndOpenDir(ContentManager.IMPORT_ROOT); },
				() => { FlansModToolbox.CreateAndOpenDir(ContentManager.MODEL_IMPORT_ROOT); },
				() => { Application.OpenURL("https://github.com/FlansMods/FlansModReloaded/wiki/Content-Tool-Tutorial-%E2%80%90-Importing-a-FM:Legacy-(1.12-and-earlier)-Pack"); },
			},
			64);
		GUILayout.Space(16);
	...
	public void ImportTab_Packs(ContentManager instance)
	{
		foreach (var kvp in instance.GetImportPackMap())
		{
			...
			if (hasFullImportMapForAllTypes)
			{
				...
				{
					instance.GenerateFullImportMap(sourcePack, true);
				}
			}
			else
			{
