...
public class JavaToCSImport : MonoBehaviour
{
	...
	public const string RootCSDir = "Assets/FlansContentTool/Scripts/Generated";
	public const string ManualOverridesDir = "Assets/FlansContentTool/Scripts/Generated/ManualOverrides";

	public string ScanDir;

	public List<string> AutoMappings;
	public List<string> AutoEnums;
	...
	public void Process()
	{
		...
		foreach(string autoEnum in AutoEnums)
		{
			string className = new FileInfo(autoEnum).Name.Split(".")[0];
			ImportEnum(autoEnum,
				$"{RootCSDir}/Enums/{className}.cs",
				className);
		}
		
	}

	private Regex GetEnumNameRegex = new Regex("\\t?([A-Za-z0-9_]+),?");
	private void ImportEnum(string javaPath, string csPath, string className)
	{
		...
		output.Add($"public enum {className}");
		output.Add("{");
		foreach(string line in input)
		{
			...
			Match match = GetEnumNameRegex.Match(line);
			if(match.Success)
			{
